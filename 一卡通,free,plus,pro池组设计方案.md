...
| 特殊接口说明 | 保持各渠道当前支持接口边界，Codex 仍按现状只支持 Responses 相关入口 | `pkg/onecard` 内的 `EndpointPolicy` 只做能力说明和错误提示 |
...
| 原生不支持 | `/v1/chat/completions` |
| 一卡通默认兼容 | Codex + `gpt-*` 的 Chat Completions 请求由系统内部转换为 Responses |
| 上游路径 | `/backend-api/codex/responses` |

一卡通项目为了兼容普通 OpenAI Chat 客户端，默认启用 Codex Chat Completions -> Responses 兼容层。注意：这不是让 Codex 上游原生支持 Chat Completions，而是 new-api 内部先把 Chat 请求转换成 Responses 请求，再把 Responses 响应转换回 Chat 响应。

该兼容能力必须封装在 OneCard 模块中，主链路只调用 OneCard 的接口能力策略，不在 controller、middleware、service 中散落 Codex 特判。
...
| 用户请求 `/v1/chat/completions` 且命中 Codex + `gpt-*` 模型 | 默认通过 OneCard 兼容层转换到 `/v1/responses` |
| 用户请求 `/v1/chat/completions` 且模型不命中兼容策略 | 返回接口不支持 |
| 用户希望直接使用 Codex 原生能力 | 推荐直接调用 `/v1/responses` 或 `/v1/responses/compact` |

### 10.2 固定接口策略

| 策略 | 决策 |
|---|---|
| 代码内置 Codex 自动 Chat -> Responses 转换 | 做，作为一卡通默认兼容能力 |
| 管理端配置 Chat -> Responses 兼容策略 | 可继续作为高级配置存在，但一卡通 Codex 默认能力不依赖管理员手动开启 |
| UI 提示 | 仍需提示 Codex 原生是 Responses-only；Chat 入口是 OneCard 兼容层能力 |
...
| 失败率 | 按池内自动禁用账号占比统计 |
| 平均延迟 | 按渠道 `response_time` 聚合展示 |
| 剩余额度/usage | 按渠道 `balance`、`used_quota` 聚合展示 |
| 自动禁用数量 | 用于告警 |
...
| 项目 | 决策 |
|---|---|
| 动态失败率降权 | 不新增独立算法，复用现有 `auto_ban`、渠道禁用和 priority/weight |
| 根据剩余额度调度 | 不新增独立算法，管理员通过 channel 状态、priority、weight 控制 |
| 根据延迟调度 | 不新增独立算法，延迟进入健康看板用于人工运营 |
| 根据成本调度 | 不新增独立算法，成本差异通过池组倍率和管理员分池体现 |

## 14. 独立功能模块与低侵入实现方案

这一节是本方案的实现约束：一卡通能力必须封装成独立功能模块，尽量少改 new-api 主流程。从主干合代码时，应像插一块乐高，而不是在老代码里撒芝麻。
...
| 账号导入格式 | `ImportParser` | `BaseImportParser` | `JSONParser` | 字段解析、凭证校验、错误提示 |
...
| `pkg/onecard/compat.go` | Chat Completions、Responses、Codex 等接口兼容策略 |
| `pkg/onecard/importer` | 账号批量导入、凭证校验、渠道生成 |
| `pkg/onecard/health.go` | 账号健康、失败率、额度、风控状态 |
...
| Importer | `Parse`、`Validate`、`CreateChannels` | JSON/sub2api 主流 JSON 格式解析，落库仍走现有渠道创建逻辑 |
...
Codex 保持当前上游接口能力边界：原生只支持 Responses 相关入口。但一卡通项目默认提供 Codex Chat Completions -> Responses 兼容层，用于兼容普通 OpenAI Chat 客户端和操练场。onecard 模块需要同时说明“原生支持哪些入口”和“哪些入口可通过兼容层支持”，避免在 relay 主流程里散落 provider 判断。
...
| `CodexEndpointPolicy` | Codex 渠道 | 原生只允许 Responses；Chat + `gpt-*` 可通过 OneCard 兼容层转 Responses |
...
具体“Codex 原生为什么不支持 Chat Completions、哪些模型允许通过兼容层转 Responses、应该使用哪个入口”的说明全部藏在 `CodexEndpointPolicy` 或 OneCard compat facade 内部。
...
| `pkg/onecard/compat.go` | 封装 Codex Chat -> Responses 默认兼容策略，并集中提供接口能力提示 | 低 | 没有策略时走原逻辑 |
...
| M7 | 健康与运营能力 | 在现有 channel/used_quota/response_time 基础上做池看板和健康评分 |
...
所以本次落地的最优路径是：**以现有主链路作为正式架构底座，在它旁边补 onecard 增强模块，而不是重做一套账号池/路由系统**。
...
| `GroupGroupRatio` | 不启用 | 一卡通按实际池组倍率计费，不叠加用户类型特殊倍率 |
...
| 写入 `ContextKeyUsingGroup` | distributor 用这个 group 选渠道 |
...
Codex 上游保持 Responses-only，但一卡通项目默认提供 `/v1/chat/completions` 到 Responses 的兼容转换。用户可以继续使用普通 OpenAI Chat 客户端调用 Codex + `gpt-*` 模型，系统内部由 OneCard 兼容层转换到 `/v1/responses`，再把响应转换回 Chat 格式。

固定产品规则：

| 规则 | 说明 |
|---|---|
| Codex 上游保持 Responses-only | 不把 Codex 渠道声明成原生支持 Chat Completions |
| OneCard 默认兼容 Chat -> Responses | Codex + `gpt-*` 的 Chat 请求默认由 OneCard 兼容层转换 |
| UI/导入规则提示接口边界 | 明确提示 Chat 入口是 OneCard 兼容能力，不是 Codex 原生能力 |
| 兼容逻辑封装在 OneCard | 主链路只调用 OneCard facade，不散落 Codex 特判 |
...
TokenAuth(existing)
  ...
  -> onecard compat/import/strategy
  -> Adaptor(existing)
```
...
本方案不新增独立 `card_orders`、`cards`、`user_cards` 表。当前项目已有的表足够承载完整闭环，避免把一卡通做成另一套孤岛账务系统。
...
| 重置规则 | 无 |
| 扣费来源 | `WalletFunding` |
| 是否影响资源池 | 不影响；永久余额只增加钱包额度，不改变 API Key 资源池 |
...
| 权益过期 | 不批量修改 API Key；请求时如果余额不足则按扣款顺序报错 |

### 16.6 扣费来源和扣款顺序

一卡通售卖闭环中，用户扣款顺序固定为：**先找一张当前余额大于 0 的权益卡；找到则本次请求完整扣这张权益卡并允许扣成负数；找不到可用权益卡时才扣永久余额；永久余额也不足时返回额度不足错误**。

本方案明确取消“多卡组合支付”：一次 API 请求只允许命中一个资金来源。命中权益卡时，本次请求只扣这一张权益卡；不拆分到第二张权益卡，也不把不足部分转给永久余额。只有在请求开始前没有任何当前余额大于 0 的权益卡时，才进入永久余额扣费。
...
| 规则 | 固定行为 |
|---|---|
| 资金来源数量 | 一次请求只使用一个资金来源 |
| 权益卡数量 | 一次请求最多扣一张权益卡 |
| 权益卡余额判断 | 只要求被选中的权益卡 `current_remain > 0` |
| 权益卡不足本次费用 | 仍然扣这张卡，允许扣成负数 |
| 是否尝试第二张权益卡 | 不尝试 |
| 是否权益卡加永久余额混合支付 | 不混合 |
| 何时扣永久余额 | 请求开始前没有任何当前余额大于 0 的权益卡时 |
| 永久余额不足 | 直接返回额度不足错误 |
...
| 7 | 允许该权益卡被扣成负数；负数表示当前 24 小时周期额度已透支，新的请求不再选择这张卡，直到下个周期重置 |
...
API Key group 回退不自动强制批量改旧 Key。Key 可以保留原 group；请求按照权益卡和永久余额扣费，余额不足时返回额度不足错误。
...
本方案目标是完整实现永久余额卡、日卡、周卡、月卡的售卖、支付、生效、扣费、退款、到期、回退、前端展示和后台运营闭环。分阶段只是开发顺序，本文列出的能力均纳入本次闭环。
...
| 单卡扣费规则 | 是 | 第一步 | 取消多卡组合支付；一次请求只扣一张当前余额大于 0 的权益卡，允许扣成负数 |
| 多权益卡扣费顺序优化 | 是 | 第一步 | 只用于选择本次请求要扣的唯一权益卡，按当前周期最早结束优先 |
...
| 独立卡券系统 | 否 | 不实施 | 现有 `TopUp + SubscriptionPlan + UserSubscription + BillingSession` 已完整表达业务闭环 |
...
| 多权益卡选择 | 按当前权益周期最早结束优先；一次请求只选择一张余额大于 0 的权益卡，并允许该卡被扣成负数 |
| 多卡组合支付 | 取消；不拆分到多张权益卡，不做“权益卡 + 永久余额”混合支付 |
| 实施策略 | 复用现有支付、订阅、计费、日志能力，当前方案内能力全部闭环 |

## 17. 开发落地清单

以下清单用于确认第 16 节定义的业务闭环已完整落地，不保留未实施的设计项。
...
| 第四步：补齐 onecard 模块化能力 | 处理规模化运营 | 商品字段 `product_type/pool_group/metadata`；池健康看板；JSON 批量导入 Codex/OpenAI/Claude/Gemini；EndpointPolicy 接口能力提示 |
...
| 问题 | 决策 |
|---|---|
| 当前项目定位 | 当前项目启动后就是“一卡通”项目，不再按纯上游 `new-api` 运行模式作为主要判断；`OneCardEnabled` 默认启用、`free/plus/pro` 默认存在、`auto = free -> plus -> pro` 都属于一卡通项目的基础设定 |
| 一卡通与基础项目的关系 | 基础项目作为底座持续合并升级；一卡通能力作为独立封装的产品层长期存在。后续 review 不再把“是否改变纯上游默认行为”作为缺陷，而是重点检查是否保持模块独立、低侵入、可合并 |
| 一卡通基础池组 | `free`、`plus`、`pro` 是一套固定资源池组，是一卡通项目的核心运行前提；它们不是可选插件，也不是从 `default` 派生出来的兼容分组 |
| `auto` 池组语义 | `auto` 固定表示按 `free -> plus -> pro` 顺序选择可用池组；这是当前一卡通项目的默认产品能力，不再兼容 `auto -> default` 这类纯上游默认语义 |
...
| Codex 是否必须兼容 `/v1/chat/completions` | 一卡通默认兼容。Codex 上游仍是 Responses-only，但 OneCard 内置兼容层会把 Codex + `gpt-*` 的 Chat Completions 请求转换为 Responses |
| 账号凭证来源格式 | 采用主流 sub2api 账号集合格式，核心为 `accounts[].credentials`；导入时要求 `accounts[].credentials.access_token`，Codex 渠道最终落库的 OAuth JSON 必须包含标准化后的 `account_id`；账号 ID 可从 `chatgpt_account_id`、`account_id` 或 access token 中解析，`refresh_token` 作为增强字段 |
...
| 一卡通模块放 `pkg` 还是 `service` | 做成独立模块，尽量避免侵入 `new-api` 现有代码，降低从主分支合代码的冲突风险 |
| 是否需要新增 onecard 专属表 | 不新增；现有 `new-api` 功能体系已经可以支持一卡通完整业务闭环 |
...
| `accounts[].credentials.refresh_token` | 增强字段 | 用于自动续签；缺失时可导入但账号长期稳定性下降 |
...
Codex `account_id` 归一化规则：

| 优先级 | 来源 | 处理结果 |
|---|---|---|
| 1 | `accounts[].credentials.account_id` | 直接写入渠道 `key` 的 `account_id` |
| 2 | `accounts[].credentials.chatgpt_account_id` | 转换为渠道 `key` 的 `account_id` |
| 3 | `accounts[].credentials.access_token` JWT claims | 从 `https://api.openai.com/auth` 内的 `chatgpt_account_id`，或兼容的扁平 claim 中解析后写入 `account_id` |
| 4 | 以上都不存在或解析失败 | 拒绝导入该账号，并提示缺少 `account_id` / `chatgpt_account_id` |

说明：`account_id` / `chatgpt_account_id` 对导入文件来说可以不是显式必填字段，因为系统允许从 access token 解析；但对最终创建出来的 Codex 渠道来说，`account_id` 是必需字段。Codex adaptor 会把它作为 `chatgpt-account-id` 请求头发给上游，因此不能只保存 `access_token`。
...
| Codex | 上游保持 Responses-only；OneCard 默认提供 Chat Completions -> Responses 兼容层，兼容普通 OpenAI Chat 客户端 |
...
该方案最大优点是：高度复用当前 new-api 现有架构，同时把新增逻辑封装在独立 onecard 模块内。老代码只增加少量 hook，业务差异通过 free/plus/pro 池组策略、provider 子类和兼容策略子类多态实现，不需要推翻现有渠道调度和计费体系，从主干合代码也更轻松。
